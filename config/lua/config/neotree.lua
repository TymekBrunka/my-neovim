  ...
  filesystem = {
    ...
    components = {
      icon = function(config, node, state)
        ...
        local luai, luahl = web_devicons.get_icon("", "lua")
        local cmakei, cmakehl = web_devicons.get_icon("", "cmake")
        local _, vimhl = web_devicons.get_icon("", "vim")
        ...
        if node.type == "directory" then
          ...
          elseif node.name == "build" then
            ...
            icon = "|󰣪"
          elseif node.name == "CMakeFiles" then
            highlight = cmakehl
            icon = "|" .. cmakei
          else
        ...
        elseif node.type == "file" then
          if success then
            ...
            if node.ext == "glsl" then
              hl = vimhl
              icon = ""
            elseif node.name == "meson.build" then
              icon = "󰣪"
